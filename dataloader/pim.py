...
def build_pim_loader(config):
    train_set, train_loader = None, None

    train_set = ImageDataset(istrain=True, root=os.path.join(config.DATA.DATA_PATH,config.DATA.TRAIN_SPLIT), data_size=config.DATA.IMG_SIZE[0])
    train_loader = torch.utils.data.DataLoader(train_set, num_workers=config.DATA.NUM_WORKERS, shuffle=True, batch_size=config.DATA.BATCH_SIZE)

    val_set, val_loader = None, None

    val_set = ImageDataset(istrain=False, root=os.path.join(config.DATA.DATA_PATH,config.DATA.VAL_SPLIT), data_size=config.DATA.IMG_SIZE[0])
    val_loader = torch.utils.data.DataLoader(val_set, num_workers=1, shuffle=True, batch_size=config.DATA.VAL_BATCH_SIZE)

    return train_set, val_set, train_loader, val_loader

def get_dataset(args):
    if args.train_root is not None:
        train_set = ImageDataset(istrain=True, root=args.train_root, data_size=args.data_size)
        return train_set
    return None
