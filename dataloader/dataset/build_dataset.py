...
def _build_dataset(config,_type='train'):
    ...
    if name == 'img':
        ...
        dataset = IterableImageDataset(parser=config.DATA.DATASET.lower(),root=config.DATA.DATA_PATH,split=split,shuffle=True,transform=transform,patch_size=config.DATA.PATCH_SIZE,stride=config.DATA.STRIDE,thumb=config.THUMB_MODE)
    elif name == 'multiview':
        ...
        if config.DATA.DATASET.lower() == 'dog':
            dataset = Dog(config.DATA.DATA_PATH,train=_type == 'train',transform=transform,target_transform=target_transform)
    else:
