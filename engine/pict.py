...
class PicTEngine(INetClsEngine):
    def __init__(self,config,**kwargs):
        ...
        self.criterion_teacher = config.PICT.TEACHER_LOSS
    ...
    def __get_pseudo_label(self,config,epoch,num_steps,idx,pl_inst_shape,targets_pl,output_pl,pl_nor_cls_index):
        ...
        #该包中局部相对病害阈值        
        if epoch >= config.PICT.INIT_STAGE_EPOCH:
        ...
        # 包中的绝对阈值
        if config.PICT.THR_ABS_UPDATE_NAME == 'fix':
            thr_min_conf = config.PICT.THR_ABS_NOR
            thr_min_dis_conf = config.PICT.THR_ABS_DIS
        elif config.PICT.THR_ABS_UPDATE_NAME == 'linear':
            thr_min_conf = 0.9 + (epoch-config.PICT.INIT_STAGE_EPOCH) / (config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH) *0.099
            thr_min_dis_conf = 0.5 + (epoch-config.PICT.INIT_STAGE_EPOCH) / (config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH) *0.499
        # sigmod函数来保证前期增加速率远远高于后期，优于线性增长
        elif config.PICT.THR_ABS_UPDATE_NAME == 'sigmod_iter':
            thr_min_conf = get_sigmod_num(config.PICT.THR_ABS_NOR_LOW,(epoch-config.PICT.INIT_STAGE_EPOCH) * num_steps + idx,(config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH) * num_steps,end=config.PICT.THR_ABS_NOR_HIGH)
            thr_min_dis_conf = get_sigmod_num(config.PICT.THR_ABS_DIS_LOW,(epoch-config.PICT.INIT_STAGE_EPOCH) * num_steps + idx,(config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH) * num_steps,end=config.PICT.THR_ABS_DIS_HIGH)
            thr_min_nor_conf = get_sigmod_num(config.PICT.THR_FIL_NOR_LOW,(epoch-config.PICT.INIT_STAGE_EPOCH) * num_steps + idx,(config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH) * num_steps,end=config.PICT.THR_FIL_NOR_HIGH,alph=5)
        elif config.PICT.THR_ABS_UPDATE_NAME == 'sigmod_epoch':
            thr_min_conf = get_sigmod_num(config.PICT.THR_ABS_NOR_LOW,epoch-config.PICT.INIT_STAGE_EPOCH,config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH,end=config.PICT.THR_ABS_NOR_HIGH)

            thr_min_dis_conf = get_sigmod_num(config.PICT.THR_ABS_DIS_LOW,epoch-config.PICT.INIT_STAGE_EPOCH,config.TRAIN.EPOCHS-config.PICT.INIT_STAGE_EPOCH,end=config.PICT.THR_ABS_DIS_HIGH,alph=10)
        ...
        if epoch >= config.PICT.INIT_STAGE_EPOCH:
            ...
            mask_ins_abs = None
            if config.PICT.THR_ABS_NOR_:
                mask_ins_abs = output_pl[:,:,pl_nor_cls_index] <= (1-thr_min_conf)
            if config.PICT.THR_ABS_DIS_:
    ...
    def cal_loss_func(self,config,models,idx,samples,targets,epoch,num_steps,criterions,**kwargs):
        ...
        if config.PICT.PERSUDO_LEARNING: 
            with torch.no_grad():
    ...
    def update_per_epoch(self,config,epoch,**kwargs):
        if config.PICT.PERSUDO_LEARNING and epoch >= config.PICT.INIT_STAGE_EPOCH:
            for i in range(len(self.thr_list)):
