...
def main(config):
    ...
    if config.TRAIN_MODE=='t_e':
        ...
        if model_ema is not None:
            ...
            loss_eval_ema, eval_metrics_ema,pred_ema,label_ema = validate(config, data_loader_test, models_without_ddp_ema,amp_autocast=amp_autocast,criterions=criterions,save_pre=True,logger=logger)
