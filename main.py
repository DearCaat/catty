            ...
            #custom model
                ...
                if persudo_inst: 
                    ...
                    # sigmod函数来保证前期增加速率远远高于后期，优于线性增长
                        ...
                        thr_min_dis_conf = get_sigmod_num(0,epoch-config.RDD_TRANS.INIT_STAGE_EPOCH,config.TRAIN.EPOCHS-config.RDD_TRANS.INIT_STAGE_EPOCH,end=0.7,alph=10)
                    ...
                    if epoch >= config.RDD_TRANS.INIT_STAGE_EPOCH:
                        ...
                        # 对于病害包来说，所有病害实例都用，只有teacher判定为正常的实例，并且其正常概率大于绝对病害阈值才纳用。对于正常包来说，其正常概率大于绝对病害阈值才纳用   & (output_pl[:,:,pl_nor_cls_index] >= thr_min_dis_conf  )
                            mask_ins = (mask_ins | (ps_mask_dis & (label_pl==pl_nor_cls_index) & (output_pl[:,:,pl_nor_cls_index] >= 0.5) )) | ((output_pl[:,:,pl_nor_cls_index] >= thr_min_nor_conf) & ps_mask_nor)
                        else:
                        #全部都用
