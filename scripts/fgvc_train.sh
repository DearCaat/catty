...
# 使用 getopt 整理参数
ARGS=$(getopt -o 'h:p:t:d:c:b:v:m::leo:' -l 'host:,project:,title:,dataset:,config:,batch-size:,val-batch-size:,multi-gpu::,multi-gpu-port::,log-wandb,ema,pin-memo,no-amp,no-val,option:' -- "$@")
...
while true ; do
     ...
     case "$1" in
          ...
          --multi-gpu-port)
               case "$2" in
                    "") CONN_MULTI_GPU_PORT=29500 ; shift 2 ;;
                    *)  CONN_MULTI_GPU_PORT=$2 ; shift 2 ;;
               esac ;;
...
echo 'multi-gpu-port:  '  "$CONN_MULTI_GPU_PORT"
...
case "$CONN_MULTI_GPU" in
    1) multi_gpu_str='';;
    '') multi_gpu_str='';;
    *) multi_gpu_str="-m torch.distributed.launch --nproc_per_node="$CONN_MULTI_GPU" --master_port="$CONN_MULTI_GPU_PORT
esac
...
python3 $multi_gpu_str ../main.py --data-path=$data_path$CONN_DATASET"/data/" --dataset=$CONN_DATASET --output=$output_path --project=$CONN_PROJECT --cfg $config $bs_str $vbs_str --title=$CONN_TITLE $log_wandb_str $ema_str $p_m_str $n_amp_str $n_val_str $opt_str